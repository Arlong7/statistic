...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($action == 'create') {
        ...
        if ($result->num_rows > 0) {
            ...
            $sql = "INSERT INTO women (date, employee_id) VALUES (?, ?)";
        ...
        if ($result->num_rows > 0) {
            ...
            $sql = "UPDATE women SET date = ?, employee_id = ? WHERE id = ?";
            $stmt = $conn->prepare($sql);
        ...
        $check_trade_union_sql = "SELECT id FROM women WHERE id = ?";
...
<body class="bg-gray-100 flex">
    ...
    <div class="container mx-auto mt-8">
        <div class="flex justify-between items-center mb-4">
            <h1 class="text-2xl font-bold">Women Management</h1>
            <button onclick="openModal('createModal')" class="bg-blue-500 text-white px-4 py-2 rounded">Add </button>
        </div>
